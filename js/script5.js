...
$$('.button').forEach(button => {
    let count = { number: 0 },
        ...
        iconDiv = button.querySelector('.icon > div'),
        arrow = button.querySelector('.icon .arrow'),
        countElem = button.querySelector('span'),
        svgPath = new Proxy({ y: null, s: null, f: null, l: null }, {
    ...
    button.addEventListener('click', e => {
        if (!button.classList.contains('loading')) {
            if (!button.classList.contains('animation')) {
                ...
                gsap.to(svgPath, { f: 2, l: 38, duration: .3, delay: .15 });
                gsap.to(svgPath, { s: .2, y: 16, duration: .8, delay: .15, ease: Elastic.easeOut.config(1, .4) });
                gsap.to(count, {
                    number: '100',
                    duration: 3.8,
                    delay: .8,
            ...
            if (!button.classList.contains('animation')) {
                ...
                gsap.to(svgPath, { f: 8, l: 32, duration: .4 });
                gsap.to(svgPath, { s: 0, y: 30, duration: .4 });
                setTimeout(() => {
